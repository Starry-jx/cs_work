...
GLint mandelSqTransf(complexNum z0, GLint maxIter) {
	complexNum z = z0;
	GLint count = 0;
	while ((z.x * z.x + z.y * z.y <= 40) && (count < maxIter)) {
		z = complexSquare(z);
		z.x += z0.x;
		z.y += z0.y;
		count++;
...
void mandelbrot(GLint nx, GLint ny, GLint maxIter) {
	...
	for(z.x=xComplexMin;z.x<xComplexMax;z.x+=zIncr.x)
		for (z.y = yComplexMin; z.y < yComplexMax; z.y += zIncr.y) {
			iterCouunt = mandelSqTransf(z, maxIter);
			if (iterCouunt >= maxIter) 
				ptColor.r = ptColor.g = ptColor.b = 0.0;
			else if (iterCouunt > (maxIter / 8)) {
				ptColor.r = 1.0;
				ptColor.g = 0.5;
				ptColor.b = 0.0;
			}
			else if (iterCouunt > (maxIter / 10)) {
				ptColor.r = 1.0;
				ptColor.g = ptColor.b = 0.0;
			}
			else if (iterCouunt > (maxIter / 20)) {
				ptColor.b = 0.5;
				ptColor.g = ptColor.r = 0.0;
			}
			else if (iterCouunt > (maxIter / 40)) {
				ptColor.b = 0;
				ptColor.g = ptColor.r = 1.0;
			}
			else if (iterCouunt > (maxIter / 100)) {
